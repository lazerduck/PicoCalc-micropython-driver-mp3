...
static struct {
    ...
    int16_t*        scratch;
    size_t          scratch_frames; // e.g., 1024
    size_t          scratch_elems;  // scratch_frames * channels
} g = {0};
...
static mp_obj_t mp3_play(void){
    ...
    mp_printf(&mp_plat_print, "play:D audio_out_start\n");
    audio_out_start();   // start PWM + DMA
    mp_printf(&mp_plat_print, "play:E started\n");
...
static mp_obj_t mp3_poll(void){
    if (g.state != S_PLAYING) return mp_const_none;
    if (!g.scratch) return mp_const_none;
    ...
    // Non-blocking push: OK if it accepts fewer than 'got' frames.
    // (For v1 we can drop any remainder; later we can keep a leftover tail.)
    (void)audio_out_push_interleaved(g.scratch, (size_t)got);
    return mp_const_none;
}
...
static mp_obj_t mp3_state(void){
    ...
    switch (g.state){
        ...
        case S_PLAYING:s="playing"; break;
